demo = {
  ...
  initContactUsMap: function() {
    var myLatlng = new google.maps.LatLng(19.0691022, -98.2359391);
    var mapOptions = {
      zoom: 8,
      center: myLatlng,
      styles: [{
...
///otro mapa  ACtual---------------------------------------------------------------------------
  initGoogleMaps: function() {
        var map = new google.maps.Map(document.getElementById('map'), {
          center: {lat: 19.0691022, lng: -98.2359391},
          //center: {lat: 40.770088, lng: -73.971146},
          zoom: 15
        });
  ...
  (function(i, marker) { 
    google.maps.event.addListener(marker,'click',function() { 
        iw.setContent( 
         ...
         '<div id="siteNotice">'+
      '</div>'+
      '<h1 id="firstHeading" class="firstHeading">'+randomMarkers[i].title+'</h1>'+
      '<div id="bodyContent">'+
      '<button onclick="vistas('+randomMarkers[i].propi+');" class="btn btn-primary">Express</button>' +
      '</div>'+
      '</div>');   
        iw.open(map, marker); 
      }); 
...
*/
    ...
    //-------------------------------------------------------------------------------------
          if (event.type == 'circle') {
            ...
            var posicionlat = event.overlay.getCenter().lat();
            var posicionlon = event.overlay.getCenter().lat();
           var token =  $("meta[name='csrf-token']").attr("content");
            //alert(token);
            $.ajax({
                  type: "POST",
                  dataType: "json",
                  url: "../public/selectcircle",
                  data: {_token: token, radio: radius, lat: posicionlat, lon: posicionlon},
                   success: function (data) {
                            

                              for (var i=0, max = data.length; i<max; i++) {
                               var reslat = data[i].cord_lat;
                               var reslon = data[i].cord_long;
                               var point = new google.maps.LatLng(data[i].cord_lat, data[i].cord_long);
                               //console.log('Marker: ' + data[i].id_referencias + ', position: ' + point);
                              //alert(data[i].clave);
                               if (google.maps.geometry.spherical.computeDistanceBetween(point, event.overlay.getCenter()) <= event.overlay.getRadius()) {
                                    var marker = new google.maps.Marker({
                               position: point,
                               map: map,
                             });
                                    ...
                                    '<div class="col-md-12">'+
                                      ...
                                      '</br>'+
                                        '<div class="card-avatar">'+
                                            '<img class="img" src="http://localhost:8080/accurate/public/material/img/examples/'+data[i].img+'">'+            
                                        '</div>'+
                                        '<div class="card-body">'+
                                          '<h6 class="card-category text-gray">'+data[i].medio+'</h6>'+
                                          '<h4 class="card-title">$3232</h4>'+
                                          '<button class="btn btn-rose btn-round" onclick="vistas('+data[i].id+');">Agregar &nbsp;<i class="material-icons">shopping_cart</i></button>'+
                                        '</div>'+
                                      '</div>'+
                                    '</div>'+
                                    '</div>');   
