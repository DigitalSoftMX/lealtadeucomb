...
public function principal(Request $request){
        ...
        $count = Tickets::select('number_valor', 'number_ticket', 'number_gas')->where('descrip', '=', "pendiente")->where('id_gas', '=', 1)->count();
        //dd($count);
       	if($count >= 1){
   	 	
   	 	$respuesta = Tickets::select('number_valor', 'number_ticket', 'number_gas')->where('descrip', '=', "pendiente")->where('id_gas', '=', 1)->get();
        $contenedor = array();
            ...
            $station = Station::where('id', '=', 1)->value('ip');
            $url = 'http://'.$station.'/sales/public/recordlealtad.php';
            //http://'.$station.'/sales/public/recordlealtad.php
            //dd($json);
            $ch = curl_init($url); 
