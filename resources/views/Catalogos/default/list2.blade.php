      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class='notifications top-right'></div>
            ...
            @endpermission
            @if($catalog->getUrlPrefix() == "userclient" || $catalog->getUrlPrefix() == "history")
                <div class="row">
...
@if($catalog->getUrlPrefix() == "userclient")
    @include("Catalogos.default.filtros.filtro")
@else
    @include("Catalogos.default.filtros.history")
@endif
    ...
    $.each(fields,function(index,value){columns.push({'data':value});});
      ...
      $('#datatables').fadeIn(1100);
                ...
                "render": function(data, type, row){ 
                    if('{{$show}}' == ""){  
                      if('{{$catalog->getUrlPrefix()}}' == "pagos"){  
                        ...
                        @endpermission
                        @permission("$fac")
                            + '<a href="'+url_fac+data+'"><span class="material-icons">fact_check</span></a>'    
                        @endpermission
                        }
                    }else{            
                        return data
        ...
        { 
            ...
            if('{{$catalog->getUrlPrefix()}}' == "adminawards"){  
                if(aData['active'] == 1){
                    $('td:eq(5)', nRow).html( 'Activo' );
                }else{
                    $('td:eq(5)', nRow).html( 'Inactivo' );
                ...
                if(aData['img'] != null){
                    $('td:eq(1)', nRow).html( '<img style="width:145px;height:118px;" src="../public/storage/premios/' + aData['img'] + '" />' );
                }
                return nRow;
            }
            ...
            if('{{$catalog->getUrlPrefix()}}' == "usuarioexchange"){
                
                if(aData['estado'] == 1){
                    $('td:eq(5)', nRow).html('<span class="badge badge-default" style="background-color:#00bcd4">Proceso</span>');
                } 
                else if(aData['estado'] == 2){
                    $('td:eq(5)', nRow).html('<span class="badge badge-default" style="background-color:#20cc20">Entregado</span>');
                }
                else if(aData['estado'] == 3){
                    $('td:eq(5)', nRow).html('<span class="badge badge-default" style="background-color:#FF0900">Cobrado</span>');
                }
                else if(aData['estado'] == 4){
                    $('td:eq(5)', nRow).html('<span class="badge badge-default" style="background-color:#FF0900">Recibido</span>');
                }
                return nRow;
            }
      ...
      $('#datatables').fadeIn(1100);
                ...
                "render": function(data, type, row){ 
                    if('{{$show}}' == ""){
                      ...
                      if('{{$catalog->getUrlPrefix()}}' == "pagos"){  
                        ...
                        @endpermission
                        @permission("$eli")
                            + '<a href="'+url_delete+data+'" ><span class="material-icons">delete</span></a>'    
                        @endpermission
                        @permission("$ver")
                            + '<a href="'+url_ver+data+'"><span class="material-icons">remove_red_eye</span></a>'    
                        @endpermission
        ...
        { 
            ...
            if('{{$catalog->getUrlPrefix()}}' == "adminawards"){  
                ...
                if(aData['img'] != null){
                    $('td:eq(1)', nRow).html( '<img style="width:145px;height:118px;" src="../storage/app/premios/' + aData['img'] + '" />' );
                }
                return nRow;
            }
